...
def login_to_flightradar(driver, username, password):
    try:
        ...
        # Klik tombol login
        login_button = wait.until(EC.element_to_be_clickable((By.XPATH, "/html/body/div[2]/div/div/div[2]/div/div/div/div[2]/div[2]/div[2]/button/div/span")))
        time.sleep(3) 
        login_button.click()
        ...
        # Klik submit
        submit_button = wait.until(EC.element_to_be_clickable((By.XPATH, "//*[@id='headlessui-disclosure-panel-v-1']/div/div/div/form/button")))
        submit_button.click()
...
def main_lion(username, password, start_date, end_date):
    ...
    try:
        login_to_flightradar(driver, username, password)
       ...
       'PK-LFO', 'PK-LKP', 'PK-LKJ', 'PK-LKQ']
        ...
        start = datetime.strptime(start_date, "%Y-%m-%d")  # Example start date
        end = datetime.strptime(end_date, "%Y-%m-%d")  # Example end date

        for fip in pelita:
            time.sleep(SLEEP_INTERVAL)
            scrape_flight_history(driver, "IP", fip, start, end)

        logging.info("Pelita flight history scraping completed successfully.")

        # Scrape flight history for each aircraft
        for fjt in lion:
            time.sleep(SLEEP_INTERVAL)
            scrape_flight_history(driver, "JT", fjt, start, end)
        logging.info("Lion flight history scraping completed successfully.")

        for fiw in wings:
            time.sleep(SLEEP_INTERVAL)
            scrape_flight_history(driver, "IW", fiw, start, end)
        logging.info("Wings flight history scraping completed successfully.")
