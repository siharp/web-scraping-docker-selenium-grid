...
def initialize_browser(host):
    ...
    driver = webdriver.Remote(
        command_executor=f"http://{host}:4444",
        options=options
    )
    return driver
...
def main():
    ...
    driver = initialize_browser(selenium_host)
    logging.info(f"Initialized browser with Selenium host: {selenium_host}")
